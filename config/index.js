...
var config = {
  ...
  registryPort: 7001,
  webPort: 7002,
  bindingHost: '127.0.0.1', // only binding on 127.0.0.1 for local access  // 0.0.0.1 开放给外部使用
  ...
  admins: {
    ...
    yuanzhian: 'yuanzhian@dingtalk.com'
  },
  ...
  // 模块文件存储，默认将发布的私有模块跟缓存公共模块存储在本地文件系统中，路径~/.cnpmjs.org/nfs ,也就是模块文件都存储在这个目录下；或者可以选择三方储存方式比如七牛等，可这里配置插件；也支持接口开发扩展储存；
  nfs: require('fs-cnpm')({
  ...
  scopes: [ '@cnpm', '@cnpmtest', '@cnpm-test', '@zcy' ], // 可以添加几个前缀，以后发布包的时候带有这些前缀的，就代表是私有包
